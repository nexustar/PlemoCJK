#!/usr/bin/env python3
"""地域別サブフォントのメタデータを仕上げる (PlemoCJK)。

fonttools_script.py が各地域の TTF を作り終えたあとに呼ぶ。やること:

1. name テーブルのファミリー名に地域を入れ直す。
   英数字側 (eng) は地域に依存しないので地域抜きの名前で作られており、
   fontTools merge は先頭フォント (= eng) の name テーブルを採用するため、
   ここで地域入りの名前に書き換える必要がある。
2. zh_CN / zh_TW / ja / ko のローカライズファミリー名を足す。
3. meta テーブルに dlng / slng を入れる (フォント選択の手掛かりになる)。
4. OS/2 の ulCodePageRange を地域に合わせる。
5. fsSelection に USE_TYPO_METRICS を立てる。
6. 1:2 幅のバリアントに hdmx を付け、head.flags に
   「命令がポイントサイズに依存する」ビットを立てる。
   3:5 幅では 5 * 半角px == 3 * 全角px の整数解が無いので付けない。

単体実行すると任意の TTF に適用できる:
    python3 plemocjk_metadata.py build/PlemoCJKSCConsole-Regular.ttf SC SCConsole Regular
"""
...
def weight_display(style: str) -> str:
    """"ThinItalic" -> "Thin Italic" (上流 PlemolJP と同じ規則)"""
...
def english_names(base_family: str, style: str) -> dict[int, str]:
    """上流 PlemolJP の edit_meta_data と同じ命名規則で name を組む"""
...
def set_name_table(
    ...
    # 上流の eng フォント由来で残っている地域無しの名前を落とす
    name.names = [
        record
...
def set_os2(font: TTFont, config: plemocjk_config.Config, region: str) -> None:
    ...
    # 縦方向のメトリクスは OS/2 の typo* を使わせる
    os2.fsSelection |= USE_TYPO_METRICS


def add_hdmx(font: TTFont, config: plemocjk_config.Config, half_width: int) -> None:
    """hdmx を付けて「半角 2 個 == 全角 1 個」をピクセル単位で保証する。

    hdmx[ppem][glyph] = セル数 * ceil(ppem / 2)
    半角 = ceil(ppem/2), 全角 = 2 * 半角 になる。
    """
    hmtx = font["hmtx"]
...
def apply(path: str, region: str, variant_tag: str, style: str) -> None:
    ...
    width_35 = variant_tag.startswith(
        config.settings.get("DEFAULT", "WIDTH_35_STR")
    )
    ...
    if not width_35:
        half_width = int(config.settings.get("DEFAULT", "HALF_WIDTH_12"))
        add_hdmx(font, config, half_width)
    font.save(path)
    font.close()
    print(
        f"metadata: {path} family='{base_family}' region={region} "
        f"hdmx={'no' if width_35 else 'yes'}"
    )

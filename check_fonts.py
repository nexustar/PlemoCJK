...
Usage:
    python3 check_fonts.py                       # build/ を自動検出
    python3 check_fonts.py --variant Console
    python3 check_fonts.py --prepared            # source/prepared/ を検査
...
def decode_range(codec: str, leads, trails, prefix: bytes = b"") -> set[int]:
    ...
    for lead in leads:
        for trail in trails:
            ...
            except Exception:  # noqa: BLE001 - 未定義の符号位置は飛ばす
                continue
...
def standard_charsets() -> dict[str, tuple[set[int], bool]]:
    ...
    return {
        ...
        # U+2985 / U+2986 (白括弧) は 4 つの IBM Plex Sans すべてに無いので
        # 完全被覆は要求しない
...
def check_metadata(
    fonts: dict[str, TTFont], config: plemocjk_config.Config, width_35: bool
) -> list[str]:
    errors = []
    log("--- metadata ---")
    for region, font in fonts.items():
        ...
        if width_35 and has_hdmx:
            errors.append(f"{region}: 3:5 variant must not carry hdmx")
        if not width_35:
            if not has_hdmx:
                errors.append(f"{region}: 1:2 variant is missing hdmx")
...
def read_ttc_layout(path: Path) -> tuple[int, dict[str, list[tuple[int, int]]]]:
    """TTC を生で読み、タグごとの (offset, length) 一覧を返す"""
...
def check_ttc(path: Path, config: plemocjk_config.Config) -> list[str]:
    ...
    # fontTools で開けるか / 各サブフォントの cmap が正しいか
    ...
    try:
        ...
        # glyf 共有率: 各サブフォントの loca から字形データの範囲を集め、
        # 重複を除いた数と総数を比べる
...
def discover(
    ...
    --variant 未指定で build/ に複数バリアントがある場合、
    (地域, ウェイト) のキーが衝突して混ざってしまうので、
    一番揃っているバリアントを 1 つ選ぶ。
    """
    ...
    for name in variants:
        ...
        for region in config.regions:
            ...
            for style in plemocjk_config.ALL_STYLES:
                path = directory / f"{config.font_name}{tag}-{style}.ttf"
                if path.is_file():
...
def main() -> int:
    ...
    parser.add_argument("--variant", default=None, help="検査するバリアント")
    parser.add_argument(
        "--dir", default=None, help="検査するディレクトリ (既定: build/)"
    )
    parser.add_argument(
        "--prepared",
        action="store_true",
        help="source/prepared/ の CJK 側入力フォントを検査する",
    )
    parser.add_argument("--ttc", action="append", default=[], help="検査する TTC")
    parser.add_argument(
        "--font-set",
        default=None,
        help="地域=パス をカンマ区切りで直接指定 (例 SC=a.ttf,TC=b.ttf)",
    )
    parser.add_argument(
        "--style", default="Regular", help="メイン検査に使うウェイト (既定: Regular)"
    )
    ...
    if not font_paths and not args.ttc:
        print(
            "ERROR: 検査対象のフォントが見つかりません。"
            "make.sh でビルドするか --dir / --font-set を指定してください。",
            file=sys.stderr,
        )
        return 2
    ...
    if font_paths:
        ...
        try:
            ...
            if not args.prepared:
                width_35 = config.settings.get("DEFAULT", "WIDTH_35_STR") in (
                    font_paths[(next(iter(fonts)), main_style)].name
                )
                errors += check_metadata(fonts, config, width_35)

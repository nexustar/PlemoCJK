#!/usr/bin/env python3
"""IBM Plex Sans SC の Text ウェイトを母版から再生成する (PlemoCJK)。

IBM が配布している IBM Plex Sans SC の Text は補間位置が 425 で作られていて、
JP / TC の Text (450) より約 5% 細い。4 地域サブフォントを並べたときに
SC だけ細くなってしまうので、IBM が公開している Glyphs 母版から
補間位置 450 で Text だけを作り直す。他のウェイトは IBM 発布件をそのまま使う。
...
1. IBM/plex の GitHub リリース (タグ `@ibm/plex-sans-sc@X.Y.Z`) の
   添付ファイル `sources.zip` を取得する。URL と SHA-256 は sources.lock に
   記録し、以後は検証する。zip は source/ 以下にキャッシュする (リポジトリには入れない)
2. zip から母版 `sources/masters/IBM Plex Sans SC.glyphs` だけを取り出す
3. name = "Text" のインスタンスの interpolationWeight を 425 から 450 に書き換える
   (Medium 505 / SemiBold 602 は母版に残った古い値なので触らない)
4. fontmake で Text インスタンスだけを TTF に補間する
5. IBM 発布件の SC Text に合わせて cmap / name / OS/2 などを揃える
   (fontmake は IBM が cmap に載せていない U+0302 / U+0303 / U+24C7 を
    足してしまうので外し、逆に IBM が持つ U+22EF を足す。
    これで SC の 8 ウェイトの cmap が完全に一致する)
6. source/regenerated/IBMPlexSansSC-Text.ttf に書き出す

中間生成物 (UFO 等) は数 GB になるので作業ディレクトリは TMPDIR 以下に作り、
終了時に削除する (--keep-work で残せる)。fontmake の実行には数分と 3GB 程度の
メモリが必要。glyphsLib が kern class の欠落を警告するが無害。

Usage:
    python3 regen_sc_text.py                   # 取得 + 再生成 (sources.lock を検証)
    python3 regen_sc_text.py --update-lock     # リリースを解決して sources.lock を更新
    python3 regen_sc_text.py --check           # 手元の zip と出力を検証するだけ
    python3 regen_sc_text.py --keep-work       # 中間生成物を残す
    python3 regen_sc_text.py --work-dir DIR    # 作業ディレクトリを指定する
"""
...
# sources.lock の中でこのスクリプトが使うキー。fetch_sources.py の "files" とは別
LOCK_KEY = "sc_master_source"

RELEASES_API = "https://api.github.com/repos/IBM/plex/releases?per_page=100"
# 実測 (2026-09): タグは "@ibm/plex-sans-sc@1.1.0"、添付は sources.zip (186,169,692 B)
RELEASE_TAG_PREFIX = "@ibm/plex-sans-sc@"
ASSET_NAME = "sources.zip"

# source/ 以下のキャッシュ名 (.gitignore 済み)
ZIP_NAME = "ibm-plex-sans-sc-sources.zip"
# zip 内の母版のパス。zip には instances/{postscript,truetype}/ の
# 補間済みインスタンスも入っているが、あちらは 425 のままなので使わない
...
EXPECTED_WEIGHT = 425
TARGET_WEIGHT = 450
# fontmake -i に渡す値 (familyName + styleName)
...
# 揃える OS/2 / hhea / post / head のフィールド
OS2_FIELDS = (
    "usWidthClass",
    "fsType",
    "fsSelection",
...
def resolve_release() -> dict[str, str]:
    """GitHub API から plex-sans-sc の最新リリースと sources.zip を探す"""
...
def save_lock(entry: dict[str, str]) -> None:
    """sources.lock の LOCK_KEY だけを差し替える (fetch_sources.py の記録は保つ)"""
    lock = load_lock()
    lock[LOCK_KEY] = entry
...
def ensure_sources_zip(
    ...
    """sources.zip を用意して SHA-256 を検証する。lock のエントリを返す"""
...
# ---------------------------------------------------------------------------
# 母版の instances ブロックを書き換える
#
# 210MB の openstep plist を丸ごと構文解析するのは重いので、
# instances 配列だけを括弧の対応を見ながら走査する。
# interpolationWeight は Medium (505) / SemiBold (602) にも出てくるが
# そちらは母版に残った古い値なので、Text のブロックの中だけを書き換える。
# ---------------------------------------------------------------------------
_PAIRS = {"(": ")", "{": "}"}


def _skip_string(text: str, index: int) -> int:
    """text[index] == '\"' のとき、対応する閉じ引用符の位置を返す"""
    i = index + 1
    while i < len(text):
        if text[i] == "\\":
            i += 2
...
def _match_bracket(text: str, index: int) -> int:
    """text[index] が '(' か '{' のとき、対応する閉じ括弧の位置を返す"""
...
def _top_level_assignments(text: str, lo: int, hi: int):
    """[lo, hi) の中の深さ 0 の `key = value;` を (key, 値の開始, 値の終了) で列挙する"""
    i = lo
    key_start = lo
    equals = None
    while i < hi:
...
def patch_text_instance(path: Path) -> None:
    """name = "Text" のインスタンスの interpolationWeight を TARGET_WEIGHT にする"""
...
def align_cmap(font: TTFont, reference: TTFont) -> tuple[list[int], list[int]]:
    """cmap のコードポイント集合を発布件に合わせる。

    fontmake は IBM が cmap に載せていないコードポイント
    (U+0302 / U+0303 / U+24C7) を足してしまう。逆に IBM は U+2026 と同じ
    グリフに U+22EF も割り当てているが fontmake はこれを作らない。
    SC の 8 ウェイトで cmap が揃っていないと prepare_cjk.py の
    回退補入の結果がウェイトごとにずれるので、ここで合わせる。

    グリフ名は発布件 (production names) と再生成 (--no-production-names) で
    一致しないものがあるので、まず「発布件で同じグリフを指す別のコードポイント」
    経由で解決し、それが無ければ名前で解決する。
    """
...
def align_names(font: TTFont, reference: TTFont) -> None:
    """nameID 255 以下を発布件のものに差し替える。

    256 以上は GSUB の FeatureParams (stylistic set の表示名など) が
    参照しているので再生成フォント側のものを残す。
    """
...
def align_tables(font: TTFont, reference: TTFont) -> list[str]:
    """OS/2 / hhea / post / head の数値を発布件に合わせる"""
    changed = []
    for tag, fields in (
        ("OS/2", OS2_FIELDS),
        ("hhea", HHEA_FIELDS),
...
def verify(config: plemocjk_config.Config, out_path: Path) -> int:
    """一 / 丨 の線幅と cmap が他地域・他ウェイトと揃っているか確かめる"""
    errors = 0
    log("--- verification ---")
...
def main() -> int:
    ...
    except Exception as exc:  # noqa: BLE001 - 利用者に見せたいのはメッセージだけ
